...
def get_diff_rebase(path=".", diff_filter="ACDMRTUXB"):
    messageNum = '.git/rebase-merge/msgnum'

    if (os.path.exists(messageNum) == False):
        return

    commitIndex = open(messageNum, 'r').read()

    arguments = [
        "git", "--no-pager", "diff", "--ignore-space-change",
        "--ignore-all-space", "--ignore-blank-lines",
        f"--diff-filter={diff_filter}",
        "@{" + str(commitIndex).strip() + "}",
    ]
...
if __name__ == "__main__":
    diff = get_diff() or get_diff_rebase()
